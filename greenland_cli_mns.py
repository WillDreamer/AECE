...
@click.option("--rollout-nodes", default=0, type=int,
              help="DISAGGREGATED mode: how many of --num-nodes serve rollout (SGLang) "
                   "instead of training. 0 (default) = colocate. E.g. --num-nodes 6 "
                   "--rollout-nodes 4 => 2 training nodes (16 GPU) + 4 rollout nodes (32 GPU).")
@click.option("--user-sim-nodes", default=1, type=int,
              help="How many of the --rollout-nodes are reserved to serve a SEPARATE "
                   ...
                   "the multi-model --sglang-config from it. Requires --rollout-nodes > 0 "
                   "(disaggregated); set 0 to disable (single-model rollout, e.g. for the "
                   "Bedrock user-sim path).")
...
def obx(script, num_nodes, rollout_nodes, user_sim_nodes, job_name, image, wandb_key, hf_token, stage_model,
    ...
    if rollout_nodes < 0:
        sys.exit("ERROR: --rollout-nodes must be >= 0")
    if user_sim_nodes < 0:
    ...
    if rollout_nodes > 0:
        ...
        # The user-sim model is carved out of the rollout nodes, so there must be
        # at least 1 rollout node left for the actor's own SGLang engines.
    ...
    elif user_sim_nodes > 0:
        # No rollout nodes means colocate, where slime locks rollout_num_gpus to
        # the actor GPUs — there is no separate pool to host the user-sim model.
        sys.exit(f"ERROR: --user-sim-nodes ({user_sim_nodes}) requires --rollout-nodes > 0 "
                 f"(disaggregated). Under colocate the rollout GPU pool equals the actor "
                 f"GPUs, leaving no room for a separate user-sim model. Pass --user-sim-nodes 0 "
                 f"to disable, or use --rollout-nodes to enter disaggregated mode.")
